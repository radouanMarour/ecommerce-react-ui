        ...
        <Link to={`/products/${_id}`} className="block shadow-md bg-white hover:shadow-lg transition-shadow duration-300 pb-2">
            <div className="mb-2 relative">
                ...
                <img
                    src={images[0]}
                    alt={name}
                    className={`w-full object-contain h-60 mb-2 ${!imageLoaded ? 'hidden' : ''}`}
            ...
            <button
                className={`w-11/12 mx-auto block border border-gray-500 text-gray-600 hover:bg-gray-600 hover:text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-colors}`}
            >
                Add to Cart
            </button>
        </Link >
